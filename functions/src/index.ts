...
const validate = async (req: any) => {
  try {
    const { contractAddress, tokenId, owner, flowOwner, expiresAt, message, signature } = getParams(req);
    ...
    if (!(await util.executeScript('check_account.cdc', [[ flowOwner, t.Address ]]))) throw new Error('Account Not Initialized');
    if (await util.executeScript('exists_replica.cdc', [[ `${contractAddress.toLowerCase()}-${tokenId}`, t.String ]])) throw new Error('Already Minted');
    return { success: true };
  } catch (e) {
...
const validateAndMint = async (req: any) => {
  try {
    ...
    const res = await util.sendTx('mint_nft.cdc', [
      ...
      [ contractAddress.toLowerCase(), t.String ], // contractAddress
      [ Number(tokenId), t.UInt64 ], // tokenId
      [ owner.toLowerCase(), t.String ], // ownerAtTime
